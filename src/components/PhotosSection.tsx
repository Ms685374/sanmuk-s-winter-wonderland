...
const initialPhotos: Photo[] = [
  { id: '1', src: couple3d, caption: 'Us Together', isPlaceholder: false },
  { id: '2', src: glassHeart, caption: 'Our Love', isPlaceholder: false },
  { id: '3', src: coupleChristmas, caption: 'Christmas Vibes', isPlaceholder: false },
  { id: '4', src: '', caption: 'Add Photo', isPlaceholder: true },
  { id: '5', src: '', caption: 'Add Photo', isPlaceholder: true },
  { id: '6', src: '', caption: 'Add Photo', isPlaceholder: true },
];
...
const PhotosSection = () => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setPhotos(prev => prev.map(photo => 
          photo.id === photoId 
            ? { ...photo, src: reader.result as string, isPlaceholder: false, caption: 'Our Memory' }
            : photo
        ));
      };
  ...
  const removePhoto = useCallback((photoId: string) => {
    setPhotos(prev => prev.map(photo =>
      photo.id === photoId
        ? { ...photo, src: '', isPlaceholder: true, caption: 'Add Photo' }
        : photo
    ));
  ...
  return (
    <section id="photos" className="py-32 px-6 relative overflow-hidden" ref={ref}>
      ...
      {/* Lightbox modal */}
      {selectedPhoto && !selectedPhoto.isPlaceholder && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-50 bg-foreground/90 backdrop-blur-xl flex items-center justify-center p-6"
          ...
          >
            ...
            <div className="absolute top-4 right-4 flex gap-2">
              <button
                ...
                className="glass w-10 h-10 rounded-full flex items-center justify-center text-foreground hover:text-destructive transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
